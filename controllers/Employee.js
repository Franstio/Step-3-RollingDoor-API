...
export const CheckTransaction = async (IdContainer)=>{
    const check = await  db.query("select cast(recorddate as datetime) from transaction where idContainer=? and cast(recordDate as datetime) >= now() - interval 1 minute order by id desc limit 0,1",{
        type: QueryTypes.SELECT,
        replacements: [IdContainer]
    });
...
export const SaveTransaksi = async (req,res) => {
    ...
    try
    {
        ...
        {
            if (await CheckTransaction(payload[i].idContainer))
...
export const SendToPIDSG = async (data)=>{
...
export const SyncTransaction = async ()=>{
    const data = await db.query("Select t.id,t.badgeId,t.status,t.isSuccess,t.containerName,t.binName,t.neto,c.weightbin,c.step2value,t.recordDate,t.loginDate from transaction t left join container c on t.idContainer=c.containerId where t.status like '%PENDING%' and  t.id not in (select id_transaction from log_record);",{type: QueryTypes.SELECT});
